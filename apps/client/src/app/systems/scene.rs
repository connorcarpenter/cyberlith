...
use game_engine::asset::{IconData, TextStyle};
...
pub fn scene_setup(
    ...
    let camera_entity = commands.spawn(camera_bundle).insert(layer).id();
}
...
pub fn scene_draw(
    ...
    // Aggregate Icons
    for (icon_handle, _style, _transform, visibility, render_layer_opt) in icons_q.iter() {
        if !visibility.visible {
            continue;
        }
        let position = Vec3::new(mouse_pos.x, mouse_pos.y, 0.0);
        info!("drawing text at position: {:?}", position);
        let style = TextStyle::new(32.0, 4.0);
        asset_manager.draw_text(&mut render_frame, icon_handle, &style, &position, render_layer_opt, "Hello, World!");
        // asset_manager.draw_icon(&mut render_frame, icon_handle, 33, transform, render_layer_opt);
    }
}
