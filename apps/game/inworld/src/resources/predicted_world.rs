...
use crate::components::{
    AnimationState, ConfirmedTileMovement, PredictedTileMovement, RenderPosition,
};
...
impl PredictedWorld {
    ...
    pub fn extract(&mut self, main: &mut World) {
        ...
        ) in character_q.iter()
        {
            ...
            let predicted_render_pos =
                RenderPosition::extract(confirmed_render_pos, old_predicted_render_pos_opt);
            let predicted_animation_state = confirmed_animation_state.clone();

            info!(
                "PredictedWorld::extract: confirmed_entity: {:?}",
                confirmed_entity
            );
            ...
            new_entities.push((
                confirmed_entity,
                (
                    predicted_tile_movement,
                    predicted_physics,
                    predicted_render_pos,
                    predicted_animation_state,
                ),
            ));
        }
