...
use game_engine::{
    ...
    naia::Tick,
    world::{
        components::{NextTilePosition, ProcessTickResult, TileMovement},
        types::Direction,
    },
};
...
impl ConfirmedTileMovement {
    ...
    pub fn recv_updated_next_tile_position(
        ...
        if self.tile_movement.is_stopped() {
            ...
            if let Some(move_dir) = Direction::from_delta(dx, dy) {
                ...
                self.buffer_updated_next_tile_position(
                    update_tick,
    ...
    fn buffer_updated_next_tile_position(&mut self, updated_tick: Tick, next_x: i16, next_y: i16) {
