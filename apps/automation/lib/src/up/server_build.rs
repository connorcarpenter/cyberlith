...
pub async fn server_build_content() -> Result<(), CliError> {
    ...
    run_command(
        "game_client",
        "cp target/game_client_bg.wasm game_client_bg.wasm",
    )
    .await?;
    ...
    run_command(
        "content_server",
        "cargo build --release --features prod --manifest-path apps/content/Cargo.toml",
    )
    .await?;
...
async fn server_build_common(dir_name: &str, app_name: &str) -> Result<(), CliError> {
    run_command(
        app_name,
        format!(
            "cargo build --release --features prod --manifest-path apps/{}/Cargo.toml",
            dir_name
        )
        .as_str(),
    )
