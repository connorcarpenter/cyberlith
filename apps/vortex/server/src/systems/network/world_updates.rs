...
impl FSWaitlist {
    ...
    fn set_parent(&mut self, entity: Option<Entity>) {
    ...
    pub(crate) fn decompose(self) -> (String, EntryKind, Option<Entity>) {
...
pub fn insert_component_events(
    ...
    for events in event_reader.iter() {
        ...
        for (user_key, entity) in events.read::<FileSystemEntry>() {
            ...
            fs_process_insert(FSWaitlistInsert::Entry(*entry.kind, (*entry.name).clone()), &user_manager, &mut git_manager, &mut fs_waiting_entities, &user_key, &entity);
        }
        ...
        for (user_key, entity) in events.read::<FileSystemRootChild>() {
            ...
            fs_process_insert(FSWaitlistInsert::Parent(None), &user_manager, &mut git_manager, &mut fs_waiting_entities, &user_key, &entity);
        }
        ...
        for (user_key, entity) in events.read::<FileSystemChild>() {
            ...
            fs_process_insert(FSWaitlistInsert::Parent(Some(entry.parent_id.get(&server).unwrap())), &user_manager, &mut git_manager, &mut fs_waiting_entities, &user_key, &entity);
        }
    }
}
...
fn fs_process_insert(
    ...
    if waitlist.is_ready() {
        ...
        fs_process_insert_complete(user_manager, git_manager, user_key, entity, insert);
    }
}

fn fs_process_insert_complete(user_manager: &UserManager, git_manager: &mut GitManager, user_key: &UserKey, entity: &Entity, entry: FSWaitlist) {
    let Some(user) = user_manager.user_info(user_key) else {
        panic!("user not found!");
    };
    let (name, kind, parent) = entry.decompose();
    git_manager.workspace_mut(user.get_username()).spawn_entity(*entity, name, kind, parent);
}
