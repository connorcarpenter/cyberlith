...
pub(crate) fn execute(world: &mut World, action: PaletteAction) -> Vec<PaletteAction> {
    let PaletteAction::EditColor(color_entity, old_color, new_color, already_edited) = action else {
    ...
    if !already_edited {
        let Ok(mut color_component) = color_q.get_mut(color_entity) else {
            panic!("Failed to get PaletteColor component for vertex entity {:?}!", color_entity);
        };
