...
use crate::app::{
    resources::{
        action::Action, action_stack::ActionStack,
        shape_manager::{CanvasShape, ShapeManager},
    },
};

pub(crate) fn execute(world: &mut World, edge_2d_entity: Entity, shape_2d_to_select_opt: Option<(Entity, CanvasShape)>) -> Vec<Action> {
    ...
    if let Some((shape_2d_to_select, shape_type)) = shape_2d_to_select_opt {
        if let Some(shape_3d_entity_to_request) = ActionStack::select_shape(
            &mut shape_manager,
