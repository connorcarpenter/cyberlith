...
impl MeshReader {
    ...
    pub fn read(
        ...
        let Ok((meta, data)) = MeshJson::read(bytes) else {
            panic!("Error reading .mesh file");
        };

        if meta.schema_version() != MeshJson::CURRENT_SCHEMA_VERSION {
