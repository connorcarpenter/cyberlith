...
pub fn ui_define() -> (String, AssetId, ETag, UiConfig) {
    // config
    let ui_name = "forgot_username_finish";
    let ui_asset_id_str = "bqgxb1"; //AssetId::gen_random().as_string(); // keep this around to generate new AssetIds if needed!
        ...
        c.add_panel().set_style(main_container_style).contents(|c| {
            ...
            c.add_panel().set_style(body_container_style).contents(|c| {
                ...
                c.add_panel()
                    ...
                    .contents(|c| {
                        c.add_text("if the email you entered is associated with a valid account,")
                ...
                c.add_panel()
                    .set_style(heading_text_line_container_style)
                    .contents(|c| {
                        c.add_text("we'll send you an email containing your username.")
                            .set_style(heading_text_style);
                    });
