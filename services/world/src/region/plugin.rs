...
use crate::region::{http_endpoints, systems, RegionManager};
...
impl Plugin for RegionPlugin {
    fn build(&self, app: &mut App) {
        ...
        let region_manager =
            RegionManager::new(registration_resend_rate, region_server_disconnect_timeout);

        app.insert_resource(region_manager).add_systems(
            Update,
            (
