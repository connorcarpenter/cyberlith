...
use session_server_naia_proto::{
    ...
    messages::{
        GlobalChatSendMessage, MatchLobbyCreate, MatchLobbyGameStart, MatchLobbyJoin,
        MatchLobbyLeave, MatchLobbySendMessage,
    },
};
...
pub fn message_events(
    ...
    for events in event_reader.read() {
        ...
        for (user_key, req) in events.read::<ClientActionsChannel, MatchLobbySendMessage>() {
            social_manager.chat_message_manager.send_lobby_chat_message(
                &mut http_client,
                &user_manager,
                social_server_url.as_ref(),
                &session_instance,
