...
use crate::{instances::SocialInstance, state::State};
...
async fn async_impl(
    ...
    let social_instance = SocialInstance::new(http_addr, http_port);

    let mut state = state.write().await;
    state.register_social_instance(social_instance);
