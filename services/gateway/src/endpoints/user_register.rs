...
pub(crate) async fn handler(
    ...
    let auth_request = UserRegisterRequest::new(&gateway_request.username, &gateway_request.email, &gateway_request.password);
    ...
    match HttpClient::send(&auth_addr, auth_port, auth_request).await {
        Ok(_auth_response) => {
            http_server::http_log_util::recv_res(host_name, auth_server, UserRegisterResponse::name());
