...
async fn async_impl(incoming_request: GatewayUserNameForgotRequest) -> Result<GatewayUserNameForgotResponse, ResponseError> {
    http_log_util::recv_req("gateway", "client", "user_name_forgot");

    http_log_util::send_req("gateway", "auth_server", "user_name_forgot");
    let auth_server_request = AuthUserNameForgotRequest::new(
        GATEWAY_SECRET,
