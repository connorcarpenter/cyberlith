...
use gateway_http_proto::{RefreshTokenGrantRequest as GatewayRefreshTokenGrantRequest, RefreshTokenGrantResponse as GatewayRefreshTokenGrantResponse};
...
async fn async_impl(incoming_request: GatewayRefreshTokenGrantRequest) -> Result<GatewayRefreshTokenGrantResponse, ResponseError> {
    http_log_util::recv_req("gateway", "client", "refresh_token_grant");

    http_log_util::send_req("gateway", "auth_server", "refresh_token_grant");
    let auth_server_request = AuthRefreshTokenGrantRequest::new(
        GATEWAY_SECRET,
        &incoming_request.refresh_token,
    );
    ...
    Ok(GatewayRefreshTokenGrantResponse::new(auth_server_response.access_token.as_str()))
}
