...
use gateway_http_proto::{
    UserRegisterConfirmRequest as GatewayUserRegisterConfirmRequest,
    UserRegisterConfirmResponse as GatewayUserRegisterConfirmResponse,
};
...
async fn async_impl(
    incoming_request: GatewayUserRegisterConfirmRequest,
) -> Result<GatewayUserRegisterConfirmResponse, ResponseError> {
    ...
    let auth_server_request =
        AuthUserRegisterConfirmRequest::new(GATEWAY_SECRET, &incoming_request.register_token);
