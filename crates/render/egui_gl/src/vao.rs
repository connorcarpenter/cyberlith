...
pub(crate) struct VertexArrayObject {
    ...
    vao: Option<crate::gl::VertexArray>,
    vbo: gl::Buffer,
...
impl VertexArrayObject {
    ...
    pub(crate) unsafe fn new(
        ...
        let vao = if supports_vao(gl) {
            ...
            logging::debug!("VAO not supported");
            None
        };
    ...
    pub(crate) unsafe fn bind(&self, gl: &gl::Context) {
    ...
    pub(crate) unsafe fn unbind(&self, gl: &gl::Context) {
        if self.vao.is_some() {
            gl.bind_vertex_array(None);
        } else {
...
fn supports_vao(gl: &gl::Context) -> bool {
    const WEBGL_PREFIX: &str = "WebGL ";
    const OPENGL_ES_PREFIX: &str = "OpenGL ES ";

    let version_string = unsafe { gl.get_parameter_string(gl::VERSION) };
    logging::debug!("GL version: {:?}.", version_string);
    ...
    if let Some(pos) = version_string.rfind(WEBGL_PREFIX) {
        ...
        if version_str.contains("1.0") {
            ...
            logging::debug!("Supported OpenGL extensions: {:?}", supported_extensions);
        ...
        // gl targets es2.0+ so we don't concern about OpenGL ES-CM,OpenGL ES-CL
