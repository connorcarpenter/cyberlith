...
pub struct SpinnerStyleMut<'a> {
    ui_config: &'a mut UiConfig,
    style_id: StyleId,
}

impl<'a> SpinnerStyleMut<'a> {
    ...
    fn get_spinner_style_mut(&mut self) -> &mut SpinnerStyle {
        if let WidgetStyle::Spinner(spinner_style) = &mut self.get_style_mut().base.widget_style {
            spinner_style
        } else {
            panic!("StyleId does not reference a SpinnerStyle");
        }
    }
    ...
    pub fn set_background_color(&mut self, color: Color) -> &mut Self {
        self.get_spinner_style_mut().background_color = Some(color);
        self
    }

    pub fn set_spinner_color(&mut self, color: Color) -> &mut Self {
        self.get_spinner_style_mut().spinner_color = Some(color);
        self
    }

    pub fn set_background_alpha(&mut self, alpha: f32) -> &mut Self {
        self.get_spinner_style_mut().set_background_alpha(alpha);
        self
    }
