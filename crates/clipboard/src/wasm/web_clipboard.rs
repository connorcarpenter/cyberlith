    ...
    pub fn try_receive_clipboard_event(&self) -> Option<WebClipboardEvent> {
        ...
        match rx.try_recv() {
            ...
            Err(err @ crossbeam_channel::TryRecvError::Disconnected) => {
                logging::error!("Failed to read a web clipboard event: {err:?}");
                None
            }
        }
    }
...
fn setup_clipboard_copy(subscribed_events: &mut SubscribedEvents, tx: Sender<WebClipboardEvent>) {
    let Some(window) = window() else {
        logging::error!("Failed to add the \"copy\" listener: no window object");
        return;
    };
    let Some(document) = window.document() else {
        logging::error!("Failed to add the \"copy\" listener: no document object");
        return;
    };
...
fn setup_clipboard_cut(subscribed_events: &mut SubscribedEvents, tx: Sender<WebClipboardEvent>) {
    let Some(window) = window() else {
        logging::error!("Failed to add the \"cut\" listener: no window object");
        return;
    };
    let Some(document) = window.document() else {
        logging::error!("Failed to add the \"cut\" listener: no document object");
        return;
    };
...
fn setup_clipboard_paste(subscribed_events: &mut SubscribedEvents, tx: Sender<WebClipboardEvent>) {
    let Some(window) = window() else {
        logging::error!("Failed to add the \"paste\" listener: no window object");
        return;
    };
    let Some(document) = window.document() else {
        logging::error!("Failed to add the \"paste\" listener: no document object");
        return;
    };

    let closure = Closure::<dyn FnMut(_)>::new(move |event: ClipboardEvent| {
        let Some(clipboard_data) = event.clipboard_data() else {
            logging::error!("Failed to access clipboard data");
            return;
        };
...
fn clipboard_copy(contents: String) {
    spawn_local(async move {
        let Some(window) = window() else {
            logging::warn!("Failed to access the window object");
            return;
        };

        let nav = window.navigator();
        let Some(clipboard) = nav.clipboard() else {
            logging::warn!("Failed to access clipboard");
            return;
        };
