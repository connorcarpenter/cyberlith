...
pub(crate) async fn task_process_async() -> Result<String, TaskError> {
    let Some(window) = window() else {
        return Err(TaskError::IoError("Failed to access the window object".to_owned()));
    };
    ...
    match JsFuture::from(promise).await {
        Ok(value) => {
            match value.as_string() {
                ...
                None => {
                    return Err(TaskError::IoError("Failed to read from clipboard: empty value".to_owned()));
                }
            }
        },
        Err(err) => {
            return Err(TaskError::IoError(format!("Failed to read from clipboard: {}", string_from_js_value(&err))));
        }
    }
}
