...
async fn connect(url: &Url) -> Result<TcpStream, ResponseError> {
    let host = url
        .host_str()
        .ok_or(ResponseError::NetworkError("invalid host".to_string()))?;
    let port = url
        .port_or_known_default()
        .ok_or(ResponseError::NetworkError("invalid port".to_string()))?;
...
fn split_response(response_str: &str) -> Result<(&str, &str, usize), ResponseError> {
    ...
    let status_and_headers = parts.next().ok_or(ResponseError::NetworkError(
        "Missing status and headers".to_string(),
    ))?;
    let mut status_and_headers_parts = status_and_headers.splitn(2, "\r\n");
    let status_line = status_and_headers_parts
        .next()
        .ok_or(ResponseError::NetworkError(
...
fn parse_status_line(status_line: &str) -> Result<(u16, String), ResponseError> {
    ...
    let _http_version = parts.next().ok_or(ResponseError::NetworkError(
        "Missing HTTP version".to_string(),
    ))?;
    let status_code = parts.next().ok_or(ResponseError::NetworkError(
        "Missing status code".to_string(),
    ))?;
    let status_text = parts.next().unwrap_or("").to_string(); // Status text can be empty
    let status_code = status_code
        .parse::<u16>()
        .map_err(|e| ResponseError::NetworkError(format!("Invalid status code: {}", e)))?;
...
fn parse_headers(headers: &str) -> Result<Vec<(String, String)>, ResponseError> {
    ...
    for line in headers.lines() {
        ...
        let key = parts
            .next()
            .ok_or(ResponseError::NetworkError(
                "Missing header key".to_string(),
            ))?
            .to_lowercase();
        let value = parts
            .next()
            .ok_or(ResponseError::NetworkError(
                "Missing header value".to_string(),
            ))?
            .to_string();
