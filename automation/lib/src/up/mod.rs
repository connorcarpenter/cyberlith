...
use crate::{
    ...
    utils::{check_channel, thread_init_1arg, thread_init_compat},
    CliError,
};
...
pub fn up() -> Result<(), CliError> {
    ...
    let (mut redirector_rdy, redirector_rcvr) = if config.contains("redirector") {
        (
            false,
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_redirector,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut gateway_rdy, gateway_rcvr) = if config.contains("gateway") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_gateway,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut content_rdy, content_rcvr) = if config.contains("content") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_content,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut auth_rdy, auth_rcvr) = if config.contains("auth") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_auth,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut region_rdy, region_rcvr) = if config.contains("region") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_region,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut session_rdy, session_rcvr) = if config.contains("session") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_session,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut world_rdy, world_rcvr) = if config.contains("world") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_world,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut asset_rdy, asset_rcvr) = if config.contains("asset") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_asset,
            )),
        )
    } else {
        (true, None)
    ...
    let (mut social_rdy, social_rcvr) = if config.contains("social") {
        (
            ...
            Some(thread_init_1arg(
                image_tag.clone(),
                server_build::server_build_social,
            )),
        )
    } else {
        (true, None)
